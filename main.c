...
/* Structure pour identifié un joueur*/
typedef struct {
    int socket_fd;
    char name[50];
} Player;
...
void broadcast_message(const char* msg, int exclude_fd){
    ...
    for (int i = 0; i < players_count; ++i) {
        if(players[i].socket_fd != exclude_fd){
            send(players[i].socket_fd, msg, strlen(msg),0);
        }
    }
...
void *handle_client(void *client_sock) {
    ...
    Player *player = &players[players_count++]; // Créer un nouveau joueur
    ...
    /* Pour l'instant, on récupère juste les messages du client et on les affiche en console */
    char buffer[BUFSIZ];
    while(1){
    ...
    pthread_mutex_lock(&client_mutex);
    for (int i = 0; i < players_count; ++i) {
        if(players[i].socket_fd == client_fd){
            players[i].socket_fd = players[--players_count].socket_fd;
            break;
        }
    }
    pthread_mutex_unlock(&client_mutex);
...
void shutdown_server(int listen_fd) {
    ...
    pthread_mutex_lock(&client_mutex);
    for (int i = 0; i < players_count; ++i) {
        send(players[i].socket_fd, shutdown_msg, strlen(shutdown_msg), 0);
        close(players[i].socket_fd);  // Fermer chaque socket client
    }
    players_count = 0;
    pthread_mutex_unlock(&client_mutex);
